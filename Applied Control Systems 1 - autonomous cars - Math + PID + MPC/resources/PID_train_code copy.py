...
t0 = 0
t_end = 5
t = np.arange(t0, t_end+dt, dt) # np.arange是前开后闭，为保证t_end也被取到，所以+dt
...
trials_magn = trials
history = np.ones(trials) # 记录是否成功接住cube
while(trials > 0): # Determines how many times cube falls down
    ...
    for i in range(1, len(t)):
        ...
        F_a = K_p*e[times][i-1] + K_d*e_dot[times][i-1] + K_i*e_int[times][i-1]   # PID控制公式
        F_net = F_a + F_ga_t
...
# callback function of animarion
def update_plot(frame):

    # train
    # platform.set_data([pos_x_train[int(frame/len_t)][frame-int(frame/len_t)*len_t]-3.1, pos_x_train[int(frame/len_t)][frame-int(frame/len_t)*len_t]+3.1],
    #                   [pos_y_train[int(frame/len_t)][frame-int(frame/len_t)*len_t], pos_y_train[int(frame/len_t)][frame-int(frame/len_t)*len_t]])
    platform.set_data([pos_x_train[int(frame/len_t)][0]-3.1, pos_x_train[int(frame/len_t)][0]+3.1],
                      [pos_y_train[int(frame/len_t)][0], pos_y_train[int(frame/len_t)][0]])

    # cube
    cube.set_data([pos_x_cube[int(frame/len_t)][frame-int(frame/len_t)*len_t]-1, pos_x_cube[int(frame/len_t)][frame-int(frame/len_t)*len_t]+1],
                  [pos_y_cube[int(frame/len_t)][frame-int(frame/len_t)*len_t], pos_y_cube[int(frame/len_t)][frame-int(frame/len_t)*len_t]])


    if (trials_magn*len_t == frame+1) and (frame > 0): # All attempts must be successful
        if sum(history) ==  0:
    ...
    displ_rail_f.set_data(t[0:(frame-int(frame/len_t)*len_t)],
                          displ_rail[int(frame/len_t)][0:(frame-int(frame/len_t)*len_t)])

    v_rail_f.set_data(t[0:(frame-int(frame/len_t)*len_t)],
                      v_rail[int(frame/len_t)][0:(frame-int(frame/len_t)*len_t)])

    a_rail_f.set_data(t[0:(frame-int(frame/len_t)*len_t)],
                      a_rail[int(frame/len_t)][0:(frame-int(frame/len_t)*len_t)])

    e_f.set_data(t[0:(frame-int(frame/len_t)*len_t)],
                 e[int(frame/len_t)][0:(frame-int(frame/len_t)*len_t)])

    e_dot_f.set_data(t[0:(frame-int(frame/len_t)*len_t)],
                     e_dot[int(frame/len_t)][0:(frame-int(frame/len_t)*len_t)])

    e_int_f.set_data(t[0:(frame-int(frame/len_t)*len_t)],
                     e_int[int(frame/len_t)][0:(frame-int(frame/len_t)*len_t)])
...
plt.xlim(0, init_pos_x_global)                                              # plt作用于刚刚创建的subplot
plt.ylim(0, init_pos_x_global)
plt.xticks(np.arange(0, init_pos_x_global+1, 10))
plt.yticks(np.arange(0, init_pos_x_global+1, 10))
plt.grid(True)
copyright = ax_main.text(0, 122, '© Mark Misin Engineering', size = 12)       # begin the 0 coordinate


# add a rail and a empty both of a train and a cube to the main window
rail = ax_main.plot([0, init_pos_x_global], [5, init_pos_x_global*np.tan(incl_angle)+5], 'k', linewidth = 6) # 两点构造直线
platform, = ax_main.plot([], [], 'b', linewidth = 18)                         # plt.plot返回的是一个数组
cube, = ax_main.plot([],[],'k', linewidth = 14)


# text for success or failure
...
plt.xlim(t0, t_end)
plt.ylim(np.min(displ_rail)-abs(np.min(displ_rail))*0.1, np.max(displ_rail)+abs(np.max(displ_rail))*0.1)   # np.max, np.max可以对嵌套列表取最大最小值！
plt.grid(True)
...
pid_ani = animation.FuncAnimation(fig,
                                update_plot,
                                frames = frame_amount,
                                interval = 20,                 # [ms]
                                repeat = False,
