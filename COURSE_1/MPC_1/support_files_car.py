...
class SupportFilesCar:
    ...
    def __init__(self):
        ...
        self.Q=np.matrix('1 0;0 1') # weights for outputs (all samples, except the last one)
        self.S=np.matrix('1 0;0 1') # weights for the final horizon period outputs
        self.R=np.matrix('1') # weights for inputs (only 1 input in our case)

        self.outputs=2 # number of outputs
        ...
        self.r=4
        self.f=0.01
        self.time_length = 10 # [s] - duration of the entire manoeuvre

        ### PID ###
        self.PID_switch=0 # Turn PID function ON/OFF (ON=1, OFF=0)
        ...
        self.trajectory=3 # You should only choose 1,2,3
    ...
    def open_loop_new_states(self,states,U1):
        ...
        sub_loop=1  #Chop Ts into 30 pieces
